...
session_start();
// 変数を初期化
$name=$email=$password=$postal_code=$address=$phone_number='';
                ...
                <fieldset>
                    ...
                    <form action="signup-output.php" method="post" class="error_check form-horizontal">
                        <div class="form-group">
                            ...
                            <input type="text" id="name" name="name" class="form-control" value="<?= $name;?>" required aria-required="true" autofocus>
                        </div>
                        <div class="form-group">        
                            <label for="email" class="control-label"><span class="required">メールアドレス（※必須）</span>：（例：suzuki@xxx.com）</label>
                            <input type="email" id="email" name="email" class="form-control col-sm-6" value="<?= $email;?>" pattern="^[a-z0-9._%+-]+@[a-z0-9.-]+\.[a-z]{2,3}$" required aria-required="true">
                        </div>
                        <div class="form-group">        
                        ...
                        <div class="form-group">
                            ...
                            <input type="text" id="postal_code" name="postal_code" value="<?= $postal_code;?>" class="form-control" pattern="\d{3}-?\d{4}" reqiured aria-requied="true"> 
                        </div>
                        <div class="form-group">
                            <label for="address" class="control-label"><span class="required">住所（※必須）</span>：（例：東京都新宿区西新宿x-x-x〇〇マンションxxx号室）</label>
                            <input type="text" id="address" name="address" value="<?= $address;?>" class="form-control" required area-required="true">
                        </div>
                        <div class="form-group">
                            <label for="phone_number" class="control-label"><span class="required">電話番号（※必須）</span>：（例：03-1234-XXXX）</label>
                            <input type="tel" id="phone_number" name="phone_number" value="<?= $phone_number;?>" class="form-control" pattern="\d{2,4}-\d{3,4}-\d{3,4}" reqiured aria-reqiured="true">
                        </div>
